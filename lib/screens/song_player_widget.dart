    ...
    return MultiValueListenableBuilder(
        ...
        builder: (context, values, child){
          return FutureBuilder(
              future: widget.controller.retrieveSong(widget.controller.controllerQueue[widget.controller.indexNotifier.value]),
              builder: (context, snapshot){
                if(snapshot.hasError){
                  ...
                  MetadataType currentSong = snapshot.data as MetadataType;
                  return Stack(
                                ...
                                SizedBox(
                                    ...
                                    child: ListView.builder(
                                      ...
                                      itemBuilder: (context, int index) {
                                        String path = widget.controller.settings.queue[index];
                                        return FutureBuilder(
                                          future: widget.controller.retrieveSong(path),
                                          builder: (context, snapshot){
                                            if(snapshot.hasData){
                                              var song = snapshot.data as MetadataType;
                                              ...
                                              return const Text("Error loading song");
                                            }
                                            else{
                                    ...
                                    child: Container(
                                      decoration: BoxDecoration(
                                          ...
                                          image: DecorationImage(
                                            ...
                                            fit: BoxFit.cover,
                                            image: widget.controller.imageNotifier.value == Uint8List(0) ? Image.asset('assets/bg.png').image : Image.memory(widget.controller.imageNotifier.value).image,
                                          )
                                      ),
                                    ),
                                  ),
