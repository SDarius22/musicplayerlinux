...
//
//   Box<AlbumType> albumBox = ObjectBox.store.box<AlbumType>();
//   Box<ArtistType> artistBox = ObjectBox.store.box<ArtistType>();
//   Box<PlaylistType> playlistBox = ObjectBox.store.box<PlaylistType>();
//   Box<Settings> settingsBox = ObjectBox.store.box<Settings>();
//   Box<SongType> songBox = ObjectBox.store.box<SongType>();
...
//   ValueNotifier<Color>(Colors.white); // Light color, for lyrics and sliders
//   ValueNotifier<Color> colorNotifier2 =
//   ValueNotifier<Color>(Colors.black); // Dark color, for background of player and window bar
//
...
//   ValueNotifier<int>(0); // index of the song in the queue that can be shuffled
...
//       }
//     });
//     settingsBox.query().watch().listen((query) {
//       settings = query.find().first;
//       hasBeenChanged.value = !hasBeenChanged.value;
//     });
//
//     // Stream<Query<Settings>> watchedQuery = settingsBox.query().watch();
//     // final sub1 = watchedQuery.listen((Query<Settings> query) {
...
//         metadatas[3].toString().substring(1, metadatas[3].toString().length - 1).split(", ");
...
//             : tags.pictures[0]
//             .toString()
//             .substring(21, tags.pictures.first.toString().length - 3)
//             .split(", ");
...
//       songBox.query().order(SongType_.title).build().find().map((e) => e.path).toList();
...
//         settings.mongoID = data['id'];
//       } else {
//         settings.fromMap(data);
//       }
...
//     Query<AlbumType> albumQuery =
//     albumBox.query(AlbumType_.name.equals(metadataVariable.album)).build();
...
//     } else {
//       print("resume");
//       await audioPlayer.play(DeviceFileSource(controllerQueue[indexNotifier.value]),
//           position: Duration(milliseconds: sliderNotifier.value), volume: volumeNotifier.value);
...
//     print("Register user");
//     var data = "{'email': '${settings.email}',"
//         "'password': '${settings.password}',"
//         "'index': ${settings.index},"
//         "'position': ${settings.position},"
//         "'deviceList': ${settings.deviceList},"
//         "'primaryDevice': ${settings.primaryDevice},"
//         "'firstTime': ${settings.firstTime},"
//         "'systemTray': ${settings.systemTray},"
//         "'fullClose': ${settings.fullClose},"
//         "'appNotifications': ${settings.appNotifications},"
//         "'deezerARL': ${settings.deezerARL},"
//         "'queueAdd': ${settings.queueAdd},"
//         "'queuePlay': ${settings.queuePlay},"
//         "'queue': ${settings.queue},"
//         "'missingSongs': ${settings.missingSongs}}";
//
//     var encryptedData = await encryptData(data);
//     print(encryptedData);
...
//     var query = songBox
//         .query(SongType_.title.contains(enteredKeyword, caseSensitive: false) |
//     SongType_.artists.contains(enteredKeyword, caseSensitive: false) |
//     SongType_.album.contains(enteredKeyword, caseSensitive: false))
//         .order(SongType_.title)
//         .build();
//     query.limit = 25;
//     return query.find();
...
//     print("Update user");
//     var data = settings.toMap().toString();
//     var encryptedData = await encryptData(data);
//     print(encryptedData);
